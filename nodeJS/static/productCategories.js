window.addEventListener("load", ()=>{
    ...
    fetch('http://localhost:8000/vendor/productCategories', {method:"GET", headers: {'Authorization' : `Bearer ${token}`}})
...
function createUser(){
    ...
    let user = {
        ...
        categoryId : document.getElementById("input_surname").value
    ...
    fetch("http://localhost:8000/vendor/productCategories/add", {
        method:"POST",
        headers:{
...
function updateTable(data){
    ...
    for(i=0;i<obrisi_buttons.length;i++){
        ...
        obrisi_buttons[i].addEventListener("click", function(){
            fetch("http://localhost:8000/vendor/productCategories/delete/" + id, {method:"DELETE", headers:{'Authorization' : `Bearer ${token}`}})
                .then(response => response.json())
                .then(data => {
                    if(data.msg){
                        alert(data.msg);
    ...
    for(i=0;i<izmeni_buttons.length;i++){
        izmeni_buttons[i].addEventListener("click", function(){
            ...
            let edit = {
                ...
                categoryId : document.getElementById("edit_input_surname").value
            };
            ...
            fetch("http://localhost:8000/vendor/productCategories/edit/"+user_id, {
                method: "PUT",
                headers: {
                    'Accept': 'application/json',
                    'Content-Type' : 'application/json',
