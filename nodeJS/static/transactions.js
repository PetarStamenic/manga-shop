window.addEventListener("load", ()=>{
    ...
    fetch('http://localhost:8000/vendor/transaction', {method:"GET", headers: {'Authorization' : `Bearer ${token}`}})
...
function createUser(){
    ...
    let user = {
        ...
        orderId : document.getElementById("input_surname").value,
        code : document.getElementById("input_email").value,
        type : document.getElementById("input_password").value,
        mode : document.getElementById("input_mobile_phone").value,
        status : document.getElementById("input_vendor").value,
    ...
    fetch("http://localhost:8000/vendor/transaction/add", {
        method:"POST",
        headers:{
...
function updateTable(data){
    ...
    for(i in data){
        let redHTML =
            ...
            `</td><td>` + data[i].orderId + `</td><td>` + data[i].code + `</td><td>` + data[i].type
            + `</td><td>` + data[i].mode + `</td><td>` + data[i].status
    ...
    for(i=0;i<obrisi_buttons.length;i++){
        ...
        obrisi_buttons[i].addEventListener("click", function(){
            fetch("http://localhost:8000/vendor/transaction/delete/" + id, {method:"DELETE", headers:{'Authorization' : `Bearer ${token}`}})
                .then(response => response.json())
                .then(data => {
                    if(data.msg){
                        alert(data.msg);
        ...
        izmeni_buttons[i].addEventListener("click", function(){
            let user_id = this.parentNode.parentNode.dataset.userid;
             ...
             let edit = {
                    ...
                    orderId : document.getElementById("edit_input_surname").value,
                    code : document.getElementById("edit_input_email").value,
                    type : document.getElementById("edit_input_password").value,
                    mode : document.getElementById("edit_input_mobile_phone").value,
                    status : document.getElementById("edit_input_vendor").value,
                ...
                fetch("http://localhost:8000/vendor/transaction/edit/"+user_id, {
                    method: "PUT",
                    headers: {
                        'Accept': 'application/json',
                        'Content-Type' : 'application/json',
